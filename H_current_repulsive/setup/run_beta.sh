...
jobname="66_mu015_nsweeps_40"  #JOBNAME importan to declare -has to be descriptive
...
for Beta_val in ${Beta_arr[@]}; do
	...
	sed "s:Beta      = 5.d0            ! Inverse temperature:Beta      = ${Beta_val}d0            ! Inverse temperature:g" parameters > "${dire}"/parameters
  cp seeds "${dire}"/seeds
