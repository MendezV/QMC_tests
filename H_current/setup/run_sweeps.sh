...
#readarray -t Nsweeps_arr < Hops.dat
Nsweeps_arr=$(awk -F= '{print $1}' Nsweeps.dat)
echo ${Nsweeps_arr}
jobname="nsweeps"  #JOBNAME importan to declare -has to be descriptive
...
for Nsweeps_val in ${Nsweeps_arr[@]}; do

	#create one temporary directory per parameter
	dire=""${dire_to_temps}"/${jobname}_${Nsweeps_val}"
	rm -rf "${dire}"
	mkdir -vp "${dire}"

	#modifying "parameter" file for the specific run
	#and moving everything to the temp directory
	sed "s:NSweep               = 200   ! Number of sweeps:NSweep               = ${Nsweeps_val}   ! Number of sweeps:g" parameters > "${dire}"/parameters
	cp seeds "${dire}"/seeds
