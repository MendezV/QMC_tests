...
#readarray -t Nbins_arr < Hops.dat
Nbins_arr=$(awk -F= '{print $1}' BinsN.dat)
echo ${Nbins_arr}
jobname="binsweeps"  #JOBNAME importan to declare -has to be descriptive
...
for Nbins_val in ${Nbins_arr[@]}; do

	#create one temporary directory per parameter
	dire=""${dire_to_temps}"/${jobname}_${Nbins_val}"
	rm -rf "${dire}"
	mkdir -vp "${dire}"

	#modifying "parameter" file for the specific run
	#and moving everything to the temp directory
	sed "s:NBin                 = 5    ! Number of bins:NBin                 = ${Nbins_val}     ! Number of bins:g" parameters > "${dire}"/parameters
  cp seeds "${dire}"/seeds
